"""
Fetch public transport stop locations from Overpass for Switzerland,
then compute nearest-stop distance for every listing in the DB.

Outputs:
  transport_locs.json       — raw OSM locations [{lat, lon}, ...]
  transport_distances.jsonl — one line per listing: {listing_id, dist_transport_km}
...
LOCS_OUT = HERE / "transport_locs.json"
DIST_OUT = HERE / "transport_distances.jsonl"
...
def fetch_locations():
    ...
    print("Fetching transport stop locations from Overpass...", flush=True)
    t0 = time.time()
    ...
    print(f"  {len(locs)} stops saved → {LOCS_OUT.name}  ({time.time()-t0:.1f}s)")
    return locs


def compute_distances(locs):
    rows = [(r["listing_id"], r["lat"], r["lon"]) for r in (json.loads(l) for l in COORDS.open())]
    print(f"Computing distances for {len(rows)} listings against {len(locs)} stops...")
    t0 = time.time()
    ...
    with DIST_OUT.open("w") as f:
        for i, (lid, lat, lon) in enumerate(rows):
            ...
            f.write(json.dumps({"listing_id": str(lid), "dist_transport_km": dist}) + "\n")
            if (i+1) % 2000 == 0:
